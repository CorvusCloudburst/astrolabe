...
/** ---------------------------------------
 * @returns todays shard info
 ** --------------------------------------- */
export function getTodaysShard() {
    const today = new Date();
    console.log(`Finding today's shard: (today's date ->) ${today}`);
...
export function getNextShard() {
    const today = new Date();
    console.log(`Finding next shard after today: ${today}`);
    return getNextShardAfter(today);
};

// Same as above, but specifically fetches red shards.
// Should probably be collapsed down into one function with a boolean flag when I have a minute.
export function getNextRedShard() {
    const today = new Date();
    console.log(`Finding next RED shard after today: ${today}`);
    return getNextRedShardAfter(today);
};
...
export function getNextShardAfter(date) {
    console.log(`Finding next shard after date: ${date}`);
    const shard = getShardOnDate(date);
    console.log(`found shard:`); console.dir(shard);
...
// Same as above, but specifically fetches red shards.
// Could probably be collapsed down into one function with a boolean flag when I have the time.
export function getNextRedShardAfter(date) {
    const shard = getShardOnDate(date);

    if (shard.skipped || shard.type === shardTypes.REGULAR) {
        const nextDay = new Date(date);
        nextDay.setDate(date.getDate()+1);
        return getNextShardAfter(nextDay);
    } else {
...
export function getShardOnDate(date) {
    ...
    const realmId = day%5; // The realm goes in order and repeats.
    console.log(`Finding shard in realm: ${realmId}`);
...
const getShardForRealm = (realmId, date) => {
    ...
    console.log(`Found group:`); console.dir(group);
    console.log(`Group Locations:`); console.dir(group.locations);

    const location = group.locations.find((location) => location.id === realmId);
    console.log(`Found location:`); console.dir(location);
...
const getTimingGroup = (day) => {
    const index = (day%timingOrder.length)-1 < 0 
        ? timingOrder.length-1 
        : (day%timingOrder.length)-1;
    return timingOrder[index];
};

// Returns the reward currency based on shard type
const getRewardCurrency = (type) => {
    return type == shardTypes.STRONG ? 'Ascended Candles' : 'Wax';
};

// Returns whether the shard falls on a skip day for its timing group
...
const dateTimesFromShardTimes = (date, group) => {
    // Use UTC for consistency - Discord can handle the UNIX timestasmps for pretty timezones for users
